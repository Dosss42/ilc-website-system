...
<head>
    ...
    <title>{{ $announcement->title }} &mdash; IEMELIF Learning Center</title>
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="mainNav">
            <ul class="navbar-nav mx-auto">
                ...
                <li class="nav-item"><a class="nav-link active" href="{{ route('announcements') }}">Announcements</a></li>
...
<div style="background:var(--ilc-blue);padding:28px 0;text-align:center;border-bottom:4px solid var(--ilc-gold);">
    ...
    <p style="color:rgba(255,255,255,0.6);font-size:12px;margin:6px 0 0;">
        ...
        <a href="{{ route('announcements') }}" style="color:rgba(255,255,255,0.5);text-decoration:none;">Announcements</a>
        <span style="margin:0 8px;color:rgba(255,255,255,0.4);">&mdash;</span>
        <span style="color:#fff;">{{ Str::limit($announcement->title, 40) }}</span>
    </p>
</div>
...
<section class="ann-page-section">
    <div class="container">
        <div class="row g-4">
            ...
            {{-- ANNOUNCEMENT BODY --}}
            <div class="col-lg-8">
                <a href="{{ route('announcements') }}" class="back-link">
                    <i class="bi bi-arrow-left"></i> Back to Announcements
                </a>

                {{-- Category badge --}}
                <span class="ann-type-pill {{ $announcement->category }}" style="margin-bottom:12px;display:inline-block;">
                    <i class="bi bi-pin-fill me-1" style="color:#c5a059;font-style:normal;"></i> {{ ucfirst($announcement->category) }}
                </span>
                ...
                <div class="ann-meta-bar">
                    <span><i class="bi bi-calendar3"></i> {{ $announcement->created_at->format('F d, Y') }}</span>
                    <span><i class="bi bi-people-fill"></i> {{ ucfirst($announcement->audience) }}</span>
                </div>

                {{-- Cover Image --}}
                @if($announcement->image)
                    <img src="{{ asset('storage/'.$announcement->image) }}" alt="{{ $announcement->title }}" class="ann-article-hero">
                @endif
                ...
                @if($relatedAnns->isNotEmpty())
                <div style="margin-top:36px;">
                    <h5 style="font-weight:700;color:#1a3a6c;margin-bottom:16px;"><i class="bi bi-megaphone me-2"></i>Related Announcements</h5>
                    @foreach($relatedAnns as $r)
                    <a href="{{ route('announcements.show', $r) }}" class="related-ann-card">
                        <div class="related-ann-date">
                            <span class="r-day">{{ $r->created_at->format('d') }}</span>
                            <span class="r-mon">{{ $r->created_at->format('M') }}</span>
                        </div>
                        <div>
                            <p class="related-ann-title">{{ $r->title }}</p>
                            <p class="related-ann-cat">{{ ucfirst($r->category) }}</p>
                        </div>
                    </a>
                    @endforeach
                </div>
            ...
            <div class="col-lg-4">
                <div class="news-sidebar">
                    <div class="sidebar-widget">
                        ...
                        <a href="{{ route('announcements') }}" class="category-item {{ !request('category') ? 'active' : '' }}">
                            <span><i class="bi bi-grid me-2" style="color:var(--ilc-gold);"></i>All Announcements</span>
                        </a>
                        @foreach(['general','academic','reminder','activity','enrollment'] as $cat)
                        <a href="{{ route('announcements', ['category' => $cat]) }}" class="category-item">
