...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/news.css') }}?v=1780250000">    {{-- news page only --}}
    <style>
        ...
        <div class="container"><div class="row g-4">
            <div class="col-lg-8">
                <div style="display:flex;gap:8px;margin-bottom:20px;">
                    @for($i=0;$i<5;$i++)<span class="skel" style="height:32px;width:68px;border-radius:20px;display:block;"></span>@endfor
                </div>
...
{{-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
     MAIN NEWS SECTION
     Left: News Articles | Right: Sidebar
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• --}}
<section class="news-page-section">
    <div class="container">
        <div class="row g-4">

            {{-- â”€â”€ LEFT: NEWS ARTICLES â”€â”€ --}}
            <div class="col-lg-8">
                ...
                {{-- =============================================
                     FILTER TABS: When connected to a database,
                     add ?category= query param to each link
                     and highlight the active one dynamically:
                     class="{{ request('category') == 'events' ? 'active' : '' }}"
                     ============================================= --}}
                <div class="news-filter-tabs">
                    <a href="{{ route('news') }}" class="filter-tab {{ !request('category') ? 'active' : '' }}">All</a>
                    <a href="{{ route('news', ['category' => 'events']) }}" class="filter-tab {{ request('category') === 'events' ? 'active' : '' }}">Events</a>
                    <a href="{{ route('news', ['category' => 'academic']) }}" class="filter-tab {{ request('category') === 'academic' ? 'active' : '' }}">Academic</a>
                    <a href="{{ route('news', ['category' => 'activity']) }}" class="filter-tab {{ request('category') === 'activity' ? 'active' : '' }}">Activity</a>
                    <a href="{{ route('news', ['category' => 'achievement']) }}" class="filter-tab {{ request('category') === 'achievement' ? 'active' : '' }}">Achievement</a>
                    <a href="{{ route('news', ['category' => 'general']) }}" class="filter-tab {{ request('category') === 'general' ? 'active' : '' }}">General</a>
                </div>

                {{-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
                     FEATURED NEWS CARD (top big one)
                â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• --}}
                {{-- =============================================
                ...
                <div class="featured-news-card" style="cursor:pointer;">
                    <div class="featured-img">
                        <div class="img-placeholder">
                            ...
                            @else
                                <img src="{{ asset('images/bg4.jpg') }}" alt="{{ $featuredNews->title }}">
                            @endif
                        </div>
                    </div>
                    <div class="featured-body">
                        <span class="news-category-badge {{ $featuredNews->category }}">{{ ucfirst($featuredNews->category) }}</span>
                        <h3>{{ $featuredNews->title }}</h3>
                        <div class="news-meta">
                            <span><i class="bi bi-calendar3"></i> {{ $featuredNews->created_at->format('F d, Y') }}</span>
                        </div>
                        <p>{{ Str::limit($featuredNews->body, 300) }}</p>
                ...
                {{-- â”€â”€ PAGINATION â”€â”€ --}}
                {{-- =============================================
                     PAGINATION: When using database, replace with:
                     {{ $news->links() }}
                     ============================================= --}}
                ...
                <div class="news-sidebar">
                    ...
                    <div class="sidebar-widget">
                        ...
                        @if(isset($recentNews) && $recentNews->isNotEmpty())
                            ...
                            <a href="{{ route('news.show', $rn) }}" style="text-decoration:none;">
                                <div class="recent-post-item">
                                    <div class="recent-post-img">
                                        @if($rn->image)
                                            <img src="{{ asset('storage/'.$rn->image) }}" alt="{{ $rn->title }}" style="width:100%;height:100%;object-fit:cover;border-radius:6px;">
                                        @else
                                            <div class="img-placeholder"><i class="bi bi-image"></i></div>
                                        @endif
                                    </div>
                                    <div class="recent-post-body">
                                        <span class="post-title">{{ Str::limit($rn->title, 50) }}</span>
                                        <div class="post-date"><i class="bi bi-calendar3" style="color:var(--ilc-gold);margin-right:3px;"></i> {{ $rn->created_at->format('M d, Y') }}</div>
                                    </div>
                                </div>
                            </a>
                            @endforeach
                        @else
                        {{-- Static fallback when no news in DB yet --}}
                        @foreach([
                        ...
                        <div class="recent-post-item">
                            <div class="recent-post-img">
                                <div class="img-placeholder"><img src="{{ asset('images/'.$staticPost['img']) }}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:6px;"></div>
                            </div>
                            <div class="recent-post-body">
                                <span class="post-title">{{ $staticPost['title'] }}</span>
                                <div class="post-date"><i class="bi bi-calendar3" style="color:var(--ilc-gold);margin-right:3px;"></i> {{ $staticPost['date'] }}</div>
                            </div>
                        </div>
                        @endforeach
                        @endif
                    ...
                    <div class="sidebar-widget">
                        ...
                        <a href="{{ route('news') }}" class="category-item {{ !request('category') ? 'active' : '' }}">
                            ...
                            <span class="cat-count">{{ isset($categories) ? $categories->sum('total') : 0 }}</span>
                        </a>
                        @if(isset($categories) && $categories->isNotEmpty())
                            @foreach($categories as $cat)
                            <a href="{{ route('news', ['category' => $cat->category]) }}" class="category-item {{ request('category') === $cat->category ? 'active' : '' }}">
                        ...
                        @else
                        {{-- Static fallback --}}
                        @foreach(['events' => 4, 'academic' => 5, 'activity' => 3, 'achievement' => 2, 'general' => 6] as $cat => $count)
                    ...
                    <div class="sidebar-widget">
                        ...
                        @if(isset($sidebarAnns) && $sidebarAnns->isNotEmpty())
                            ...
                            <a href="{{ route('announcements.show', $ann) }}" style="text-decoration:none;">
                                <div class="ann-widget-item">
                                    ...
                                    <div class="ann-widget-title">{{ Str::limit($ann->title, 50) }}</div>
                                </div>
                            </a>
                            @endforeach
                        @else
                        ...
                        <div class="ann-widget-item">
                            <div class="ann-widget-date">
                                <span class="ann-day">{{ $sa['day'] }}</span>
                                <span class="ann-mon">{{ $sa['mon'] }}</span>
                            </div>
                            <div class="ann-widget-title">{{ $sa['title'] }}</div>
                        </div>
                        @endforeach
                        @endif
                    </div>
