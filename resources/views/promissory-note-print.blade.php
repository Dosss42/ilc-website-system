...
<head>
    ...
    <title>Promissory Note — {{ $note->reference_number }}</title>
    <style>
...
<div class="no-print" style="background:#1a3a6c;color:#fff;padding:10px 20px;border-radius:6px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;">
    <span style="font-weight:600;font-size:13px;">Promissory Note — {{ $note->reference_number }}</span>
...
<div class="header">
    ...
    <div class="ref-num">Reference No.: {{ $note->reference_number }}</div>
</div>

<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
    <div>
        <span style="font-size:11px;color:#888;">Date Issued:</span>
        <span style="font-weight:700;margin-left:6px;">{{ $note->date_issued->format('F d, Y') }}</span>
    </div>
    <div>
        <span style="font-size:11px;color:#888;">Status:</span>
        <span class="status-badge status-{{ $note->status }}" style="margin-left:6px;">{{ ucfirst($note->status) }}</span>
    </div>
</div>
...
<div class="info-grid">
    <div class="info-item">
        ...
        <div class="info-value">{{ $note->student->name ?? '—' }}</div>
    </div>
    <div class="info-item">
        <div class="info-label">Parent / Guardian</div>
        <div class="info-value">{{ $note->parent_guardian ?: '—' }}</div>
    </div>
    <div class="info-item">
        <div class="info-label">Grade Level</div>
        <div class="info-value">{{ ucfirst(str_replace(['grade','nursery','kindergarten'], ['Grade ','Nursery','Kindergarten'], $note->enrollment->student_data['grade_level'] ?? '—')) }}</div>
    </div>
    <div class="info-item">
        <div class="info-label">School Year</div>
        <div class="info-value">S.Y. {{ $note->enrollment->school_year ?? '—' }}</div>
    </div>
    <div class="info-item">
        <div class="info-label">Enrollment Reference</div>
        <div class="info-value">{{ $note->enrollment->reference_number ?? '—' }}</div>
    </div>
    <div class="info-item">
        <div class="info-label">Received By</div>
        <div class="info-value">{{ $note->createdBy->name ?? '—' }}</div>
    </div>
</div>

<div class="amount-box">
    <div class="amount-label">Amount Promised to Pay</div>
    <div class="amount-value">₱{{ number_format($note->amount_promised, 2) }}</div>
    @if($note->amount_overdue > 0)
        <div class="amount-sub">Outstanding balance at time of note: ₱{{ number_format($note->amount_overdue, 2) }}</div>
    @endif
</div>

<div class="promise-box">
    <div class="promise-label">⏰ Promised Payment Date</div>
    <div class="promise-date">{{ $note->promise_date->format('F d, Y') }}</div>
    @if($note->extended_date)
        <div style="font-size:11px;color:#888;margin-top:4px;">Extended to: {{ $note->extended_date->format('F d, Y') }}</div>
    @endif
    @if($note->fulfilled_at)
        <div style="font-size:11px;color:#2e7d32;margin-top:4px;font-weight:700;">✓ Fulfilled on {{ $note->fulfilled_at->format('F d, Y') }}</div>
    @endif
</div>
...
<div class="signatures">
    <div>
        ...
        <div class="sig-line">
            <div class="sig-name">{{ $note->parent_guardian ?: '________________________' }}</div>
    ...
    <div>
        <div style="height:50px;"></div>
        <div class="sig-line">
            <div class="sig-name">{{ $note->createdBy->name ?? '________________________' }}</div>
...
<div class="footer">
    ...
    <span>{{ $note->reference_number }} | Printed: {{ now()->format('F d, Y') }}</span>
</div>
