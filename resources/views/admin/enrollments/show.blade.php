...
<head>
    ...
    <title>Enrollment Details - {{ $enrollment->reference_number }}</title>
...
<body>
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">{{ $enrollment->reference_number }}</li>
                    </ol>
                </nav>
                    ...
                    <div class="col-md-8">
                        <div class="card mb-4">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <small class="text-muted">Ref: {{ $enrollment->reference_number }}</small>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p><strong>Full Name:</strong> {{ $enrollment->full_name }}</p>
                                        <p><strong>Gender:</strong> {{ ucfirst($enrollment->student_data['gender']) }}</p>
                                        <p><strong>Date of Birth:</strong> {{ \Carbon\Carbon::parse($enrollment->student_data['birthdate'])->format('F d, Y') }}</p>
                                        <p><strong>Place of Birth:</strong> {{ $enrollment->student_data['place_of_birth'] }}</p>
                                        <p><strong>Religion:</strong> {{ $enrollment->student_data['religion'] ?: 'N/A' }}</p>
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <p><strong>Last School:</strong> {{ $enrollment->student_data['last_school'] ?: 'N/A' }}</p>
                                        <p><strong>Contact:</strong> {{ $enrollment->student_data['contact'] }}</p>
                                        <p><strong>Email:</strong> {{ $enrollment->student_data['student_email'] }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                ...
                                <p class="ms-3">
                                    {{ $enrollment->student_data['street_address'] }}, {{ $enrollment->student_data['barangay'] }}<br>
                                    {{ $enrollment->student_data['city'] }}, {{ $enrollment->student_data['province'] }}<br>
                                    {{ $enrollment->student_data['region'] }} {{ $enrollment->student_data['zip_code'] ? $enrollment->student_data['zip_code'] : '' }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p><strong>Guardian Name:</strong> {{ $enrollment->student_data['guardian_name'] }}</p>
                                        <p><strong>Relationship:</strong> {{ ucfirst($enrollment->student_data['relationship']) }}</p>
                                        <p><strong>Occupation:</strong> {{ $enrollment->student_data['guardian_occupation'] ?: 'N/A' }}</p>
                                    </div>
                                    <div class="col-md-6">
                                        <p><strong>Mobile:</strong> {{ $enrollment->student_data['guardian_phone'] }}</p>
                                        <p><strong>Email:</strong> {{ $enrollment->student_data['guardian_email'] ?: 'N/A' }}</p>
                                        <p><strong>Facebook:</strong> {{ $enrollment->student_data['guardian_facebook'] ?: 'N/A' }}</p>
                                    </div>
                                </div>
                                @if($enrollment->student_data['guardian_address'])
                                    <p><strong>Guardian Address:</strong> {{ $enrollment->student_data['guardian_address'] }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                ...
                                @if($enrollment->payment_amount)
                                    <p><strong>Amount:</strong> PHP {{ number_format($enrollment->payment_amount, 2) }}</p>
                                @endif
                                @if($enrollment->payment_method)
                                    <p><strong>Method:</strong> {{ ucfirst($enrollment->payment_method) }}</p>
                                @endif
                                @if($enrollment->payment_reference)
                                    <p><strong>Reference:</strong> {{ $enrollment->payment_reference }}</p>
                                @endif
                            ...
                            <div class="card-body">
                                <div class="timeline">
                                    <div class="timeline-item">
                                        ...
                                        <strong>{{ $enrollment->created_at->format('M d, Y h:i A') }}</strong>
                                    </div>
                                    @if($enrollment->approved_at)
                                        <div class="timeline-item">
                                            <small class="text-muted">Approved</small><br>
                                            <strong>{{ $enrollment->approved_at->format('M d, Y h:i A') }}</strong>
                                        </div>
                                    @endif
                                    @if($enrollment->declined_at)
                                        <div class="timeline-item">
                                            <small class="text-muted">Declined</small><br>
                                            <strong>{{ $enrollment->declined_at->format('M d, Y h:i A') }}</strong>
                                        </div>
                                    @endif
                                </div>
                            </div>
                ...
                <form action="{{ route('admin.enrollments.approve', $enrollment) }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="alert alert-info">
                            ...
                            A student account will be created and login credentials will be sent to {{ $enrollment->student_data['student_email'] }}
                        </div>
                    </div>
    ...
    <div class="modal fade" id="declineModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('admin.enrollments.decline', $enrollment) }}" method="POST">
                    @csrf
                    <div class="modal-header">
