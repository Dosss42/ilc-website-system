...
<div class="quick-actions">
    <a href="{{ route('finance.payments.index') }}?status=pending" class="quick-action-card">
        <div class="quick-action-icon" style="background: #fff3e0; color: #e65100;">
            <i class="bi bi-hourglass-split"></i>
        </div>
        <div class="quick-action-info">
            <h4>Verify Payments</h4>
            <p>{{ $stats['pending_verification'] }} pending verification</p>
        </div>
    </a>

    <a href="{{ route('finance.installments.index') }}?overdue=yes" class="quick-action-card">
        <div class="quick-action-icon" style="background: #ffebee; color: #c62828;">
            <i class="bi bi-exclamation-triangle"></i>
        </div>
        <div class="quick-action-info">
            <h4>Overdue Payments</h4>
            <p>{{ $stats['overdue_installments'] }} students overdue</p>
        </div>
    </a>

    <a href="{{ route('finance.fees.index') }}" class="quick-action-card">
...
<div class="stats-grid">
    <div class="stat-card">
        ...
        <div class="stat-value">{{ $stats['total_collected_today'] }}</div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-value">{{ $stats['total_collected_month'] }}</div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-value">{{ $stats['total_enrolled_students'] }}</div>
    ...
    <div class="col-lg-6">
        <div class="content-card">
            <div class="card-header">
                ...
                <a href="{{ route('finance.payments.index') }}?status=pending" class="btn-action btn-view">
                    View All
                </a>
            </div>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            @forelse($pendingPayments as $payment)
                                <tr>
                                    <td>
                                        <div style="font-weight: 600;">{{ $payment->user->name ?? 'N/A' }}</div>
                                        <div style="font-size: 11px; color: #666;">{{ $payment->enrollment->grade_level ?? '' }}</div>
                                    </td>
                                    <td style="font-weight: 600; color: var(--blue);">
                                        {{ $payment->description ?? 'Payment' }}
                                    </td>
                                    <td>
                                        @if(strpos($payment->description, 'GCash') !== false)
                                    ...
                                    <td>
                                        <div style="display: flex; gap: 8px;">
                                            <a href="{{ route('finance.payments.details', $payment) }}" class="btn-action btn-view" title="View Details">
                                                <i class="bi bi-eye"></i>
                                            </a>
    ...
    <div class="col-lg-6">
        <div class="content-card">
            <div class="card-header">
                ...
                <a href="{{ route('finance.payments.index') }}?status=approved" class="btn-action btn-view">
                    View All
                </a>
            </div>
            <div class="card-body" style="padding: 0;">
                            ...
                            @forelse($recentTransactions as $transaction)
                                <tr>
                                    <td>
                                        ...
                                        <div style="font-size: 11px; color: #666;">{{ $transaction->enrollment->grade_level ?? '' }}</div>
                                    </td>
                                    <td style="font-weight: 600; color: var(--blue);">
                                    ...
                                    <td>
                                        <span style="font-size: 12px; color: #666;">
                                            {{ $transaction->reviewedBy->name ?? 'System' }}
                                        </span>
                                    </td>
                                    <td style="color: #666;">
                                        {{ $transaction->reviewed_at->format('M d, Y') ?? $transaction->updated_at->format('M d, Y') }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
...
<div class="content-card">
    <div class="card-header">
        ...
        <a href="{{ route('finance.installments.index') }}" class="btn-action btn-view">
            ...
            <div class="col-md-4">
                <div style="text-align: center; padding: 20px; background: #f8f9fa; border-radius: 10px;">
                    <div style="font-size: 36px; font-weight: 700; color: var(--blue);">
                        {{ $installmentOverview['total_installment_students'] }}
                    </div>
            ...
            <div class="col-md-4">
                <div style="text-align: center; padding: 20px; background: #fff3e0; border-radius: 10px;">
                    <div style="font-size: 36px; font-weight: 700; color: #e65100;">
                        {{ $installmentOverview['due_this_week'] }}
                    </div>
