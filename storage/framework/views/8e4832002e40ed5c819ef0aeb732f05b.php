...
<div style="display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-bottom:24px;">
    ...
    <div class="stat-card">
        ...
        <div>
            <div class="stat-value"><?php echo e($stats['partial'] + $stats['pending']); ?></div>
    ...
    <div class="stat-card">
        ...
        <div>
            <div class="stat-value">₱<?php echo e(number_format($stats['total_balance'], 0)); ?></div>
...
<div style="display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-bottom:24px;">
    <div style="background:#fff; border-radius:12px; padding:16px 20px; box-shadow:0 2px 8px rgba(0,0,0,.05); display:flex; align-items:center; gap:14px;">
        ...
        <div>
            <div style="font-size:22px;font-weight:700;color:var(--blue);"><?php echo e($stats['cash']); ?></div>
    ...
    <div style="background:#fff; border-radius:12px; padding:16px 20px; box-shadow:0 2px 8px rgba(0,0,0,.05); display:flex; align-items:center; gap:14px;">
        ...
        <div>
            <div style="font-size:22px;font-weight:700;color:var(--blue);"><?php echo e($stats['installment']); ?></div>
...
<div class="content-card mb-4">
    ...
    <form method="GET" action="<?php echo e(route('finance.students.index')); ?>" style="padding:16px 20px; display:flex; flex-wrap:wrap; gap:12px; align-items:end;">
        ...
        <div style="min-width:160px;">
            ...
            <select name="plan" class="form-select" style="font-size:13px; padding:8px 12px;">
                <option value="all"         <?php echo e($planFilter === 'all'         ? 'selected' : ''); ?>>All Plans</option>
                <option value="cash"        <?php echo e($planFilter === 'cash'        ? 'selected' : ''); ?>>Plan A – Cash</option>
                <option value="installment" <?php echo e($planFilter === 'installment' ? 'selected' : ''); ?>>Plans B/C/D – Installment</option>
            </select>
        </div>
        <div style="min-width:150px;">
            ...
            <select name="status" class="form-select" style="font-size:13px; padding:8px 12px;">
                ...
                <option value="paid"    <?php echo e($statusFilter === 'paid'    ? 'selected' : ''); ?>>Fully Paid</option>
        ...
        <div style="min-width:150px;">
            ...
            <select name="school_year" class="form-select" style="font-size:13px; padding:8px 12px;">
                <option value="all" <?php echo e($yearFilter === 'all' ? 'selected' : ''); ?>>All Years</option>
                <?php $__currentLoopData = $schoolYears; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $yr): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($yr); ?>" <?php echo e($yearFilter === $yr ? 'selected' : ''); ?>><?php echo e($yr); ?></option>
        ...
        <div style="display:flex; gap:8px;">
            ...
            <a href="<?php echo e(route('finance.students.index')); ?>" class="btn-dash btn-secondary" style="padding:9px 14px; font-size:13px;">
...
<div class="content-card">
    <div class="content-card-header">
        ...
        <span style="font-size:12px;color:var(--muted);"><?php echo e($stats['total']); ?> student(s) found</span>
    </div>
    <div style="overflow-x:auto;">
        <table class="dash-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <div style="font-weight:600;color:var(--text);"><?php echo e($s->user->name); ?></div>
                        <div style="font-size:11px;color:var(--muted);"><?php echo e($s->user->email); ?></div>
                    </td>
                    <td><span class="grade-chip"><?php echo e(ucfirst($s->grade_level)); ?></span></td>
                    <td style="font-size:12px;color:var(--muted);"><?php echo e($s->school_year); ?></td>
                    <td>
                        <?php if($s->is_cash): ?>
                        ...
                        <?php elseif($s->payment_option !== '—'): ?>
                            <span style="display:inline-flex;align-items:center;gap:5px;background:#e8f0fb;color:var(--blue);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;border:1px solid #bfdbfe;">
                                <i class="bi bi-calendar-month"></i> Plan <?php echo e($s->payment_option); ?>
                    ...
                    <td style="font-weight:600;color:var(--text);">
                        <?php echo e($s->total_fee > 0 ? '₱' . number_format($s->total_fee, 2) : '—'); ?>

                    </td>
                    <td style="font-weight:600;color:#16a34a;">
                        <?php echo e($s->amount_paid > 0 ? '₱' . number_format($s->amount_paid, 2) : '₱0.00'); ?>

                    </td>
                    <td>
                        <?php if($s->balance <= 0 && $s->amount_paid > 0): ?>
                            <span style="color:#16a34a;font-weight:700;font-size:13px;">₱0.00</span>
                        <?php elseif($s->total_fee <= 0): ?>
                            <span style="color:var(--muted);font-size:12px;">Not set</span>
                        <?php else: ?>
                            <span style="color:#dc3545;font-weight:700;font-size:13px;">₱<?php echo e(number_format($s->balance, 2)); ?></span>
                        <?php endif; ?>
                    </td>
                    <td>
                    ...
                    <td style="white-space:nowrap;">
                        <?php if($s->enrollment_id && $s->balance > 0): ?>
                            <button type="button" class="btn-dash btn-primary"
                                style="padding:6px 14px;font-size:12px;border-radius:8px;"
                                onclick="openPayModal(<?php echo e($s->enrollment_id); ?>, '<?php echo e(addslashes($s->user->name)); ?>', <?php echo e($s->balance); ?>, <?php echo e($s->total_fee); ?>, <?php echo e($s->amount_paid); ?>, '<?php echo e($s->is_cash ? 'Plan A – Cash' : 'Plan ' . $s->payment_option); ?>')">
                                <i class="bi bi-cash-stack"></i> Pay
                            </button>
                        <?php elseif($s->balance <= 0 && $s->amount_paid > 0): ?>
...
<?php echo $__env->make('finance.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ron28\Desktop\ILC SYSTEM\ilc-website-system\resources\views/finance/students.blade.php ENDPATH**/ ?>